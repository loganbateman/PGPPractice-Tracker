...
def _http_get(url: str, *, timeout: int, headers: dict, retries: int = 3) -> requests.Response:
    ...
    if last_error is not None:
        raise last_error
    raise requests.RequestException(f"Unknown HTTP failure for {url}")
...
def collect_attendance(
    ...
    max_workers = min(4, max(1, len(session_links)))
